create table if not exists public.calendar_mutation_requests (
  id uuid primary key default gen_random_uuid(),
  business_id uuid not null references public.business(id) on delete cascade,
  idempotency_key uuid not null,
  operation text not null check (
    operation in ('create', 'update', 'move', 'resize', 'delete', 'lock', 'unlock')
  ),
...
revoke all on table public.calendar_mutation_requests from anon, authenticated;

alter table public.optimization_changes
  alter column appointment_id drop not null;

create index if not exists appointments_active_calendar_idx
  on public.appointments (business_id, appointment_date, start_time)
  where deleted_at is null and status <> 'cancelled'::public.appointment_status;
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'values', p_values,
      'confirm_warnings', to_jsonb(coalesce(p_confirm_warnings, '{}'::text[]))
    ),
    auth.uid()
  )
      ...
      if exists (
        ...
        if not exists (
          ...
          v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
            ...
            'level', 'warning',
            'message', 'This weekday is outside the patient availability preference.'
          ));
        elsif not exists (
          select 1
            from public.patient_availability availability
          ...
          v_warnings := v_warnings || jsonb_build_array(jsonb_build_object(
            ...
            'level', 'warning',
            'message', 'This time is outside the patient availability preference.'
          ));
        end if;
      end if;

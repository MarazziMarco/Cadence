...
test_plan:
  ...
  - task: "Scheduler heuristic engine + non-destructive Optimization Preview (Accept/Reject, persists optimization_runs + optimization_changes)"
    implemented: true
    working: true
    file: "lib/scheduler/engine.ts, lib/api/scheduler.ts, components/scheduler/scheduler-client.tsx"
    stuck_count: 0
    priority: "high"
    needs_retesting: true
    status_history:
        -working: true
        -agent: "main"
        -comment: "Browser-verified preview: idle saved 2h, revenue +€50, waiting filled 1, moved 3, 0 violations, per-move plain-language explanations incl VIP protection. Hard constraints (working hours, lunch, no-overlap, locked, patient availability) enforced by construction. Runs+changes stored. Accept applies to appointments + marks waiting matched; Reject sets result=discarded."

agent_communication:
    -agent: "main"
    -message: "Modules 1-9 done. Scheduler + Optimization Preview verified in browser. Please run AUTOMATED FRONTEND TESTS across all major workflows using demo login owner@cadencetest.com / Test123456!. CRITICAL TIPS: (a) login inputs #email/#password must be filled with per-character typing (.type()/press_sequentially), NOT instant fill, or the value won't register; (b) dev server is slow on first compile per route (allow 30-45s) and may return a transient 502 during Next memory-restart (retry once). Test: 1) Login -> /dashboard shows real KPIs (Today's appointments, Occupancy %, Idle, Revenue) + Today's schedule list. 2) Patients: search 'Paola', filter tabs All/VIP/Archived, create client, edit via kebab, VIP toggle, open profile page. 3) Services: cards grouped by category, create service, toggle active. 4) Working Hours: toggle a day + Save, edit defaults + Save, add+delete holiday. 5) Calendar: week/day toggle, appointment blocks visible, click empty slot opens New dialog, open existing appt to edit. 6) Waiting List: shows Marco, add entry, edit, delete. 7) AI Assistant: type 'Paola can come Monday morning', click Parse with AI, wait up to 20s, verify a parsed command card appears with Monday badge; click Apply -> success toast. 8) Scheduler: pick today's date, click 'Optimize day', wait up to 15s, verify Preview with KPI cards + Moved list + explanations; click Reject (to avoid mutating). Report pass/fail per workflow with console/network errors."
    -agent: "testing"

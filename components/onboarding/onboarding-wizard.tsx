    ...
    <div className="min-h-screen bg-muted/30">
      <div className="mx-auto flex min-h-screen max-w-2xl flex-col px-4 py-8">
            ...
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}
              ...
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {step === 0 && 'Tell us who you are. You can change all of this later.'}
                  {step === 1 && 'When are you open? The scheduler always respects these.'}
                  {step === 2 && 'Set sensible defaults for new appointments.'}
                  {step === 3 && 'A couple of final touches.'}
                </p>
              </div>
              ...
              {step === 0 && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2"><Label>First name</Label><Input value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Anna" /></div>
                    <div className="space-y-2"><Label>Last name</Label><Input value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Rossi" /></div>
                  </div>
                  <div className="space-y-2"><Label>Business name *</Label><Input value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Rossi Physiotherapy" /></div>
                  ...
                  {WEEKDAYS.map((d) => (
                    <div key={d} className="rounded-lg border border-border p-3">
                      <div className="flex items-center justify-between">
                        ...
                        {!days[d].is_open && <span className="text-xs text-muted-foreground">Closed</span>}
                      </div>
                      {days[d].is_open && (
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select value={duration} onValueChange={setDuration}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>{['15','20','30','45','60','90'].map((m) => <SelectItem key={m} value={m}>{m} minutes</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label>Maximum appointments per day <span className="text-muted-foreground">(optional)</span></Label>
                    <Input type="number" min="1" value={maxDaily} onChange={(e) => setMaxDaily(e.target.value)} placeholder="No limit" />
                  </div>
                </div>
              )}
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          {step < STEPS.length - 1 ? (
            <Button onClick={() => setStep((s) => s + 1)} disabled={!canProceed}>Continue <ChevronRight className="ml-1 h-4 w-4" /></Button>
          ) : (

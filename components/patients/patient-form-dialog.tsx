      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2"><Switch checked={isVip} onCheckedChange={setIsVip} /><Label>VIP</Label></div>
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={() => mutation.mutate()} disabled={!firstName.trim() || mutation.isPending}>
            {mutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{editing ? 'Save' : 'Add client'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

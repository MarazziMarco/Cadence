...
export function MovedMessages({ businessId, changes }: { businessId: string; changes: any[] }) {
  ...
  async function prepare() {
    ...
    try {
      ...
      const list: Msg[] = acceptedMoves.map((c) => {
        const name = c.patients?.first_name || c.patients?.full_name || 'Cliente'
      ...
      toast.error(e.message || 'Errore nel preparare i messaggi')
      setPreparing(false)
  ...
  async function copy(m: Msg) {
    ...
    setCopiedId(m.id); toast.success('Copiato ✓')
    ...
    return (
      <div className="mt-4 flex flex-col gap-3 rounded-xl border border-primary/30 bg-accent/40 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span><span className="font-semibold">{acceptedMoves.length} pazienti spostati.</span> Vuoi preparare i messaggi da inviare?</span>
        </div>
        <Button size="sm" onClick={prepare} className="shrink-0">Prepara messaggi</Button>
      </div>
    )
  }
  ...
  return (
    <div className="mt-4 space-y-2">
      <p className="text-sm font-semibold">Messaggi da copiare ({acceptedMoves.length})</p>
      {loading && <div className="flex items-center gap-2 py-4 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Preparo i messaggi…</div>}
      {!loading && (msgs ?? []).map((m) => (
